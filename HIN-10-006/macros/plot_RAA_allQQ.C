...
TGraphErrors *pgStar;
TGraphErrors *pgStarUps;
TGraphAsymmErrors *pgStarUps_box;

void load_PHENIX();
void load_STAR();

void plot_cent_allQQ(bool isPaper=false, bool plotPhenix=true, bool plotStar=false)
{
  gStyle->SetOptFit(0);
  gStyle->SetOptStat(0);
  ...
  double inclJpsiRAA[7] = {0.306, 0.223, 0.241, 0.311, 0.482, 0.671, 0.595};
  double promptJpsiRAA[7] = {0.296, 0.196, 0.215, 0.362, 0.493, 0.644, 0.593};
  ...
  double inclJpsiRAAError[7] = {0.021, 0.026, 0.032, 0.045, 0.070, 0.107, 0.103};
  double promptJpsiRAAError[7] = {0.025, 0.029, 0.036, 0.056, 0.083, 0.127, 0.119};
  ...
  double inclJpsiRAASyst[7] = {0.028, 0.020, 0.022, 0.030, 0.058, 0.099, 0.118};
  double promptJpsiRAASyst[7] = {0.026, 0.016, 0.019, 0.035, 0.057, 0.090, 0.112};
  ...
  TGraphErrors *gInclJpsiRAA = new TGraphErrors(7, promptNpart, inclJpsiRAA, promptNpartError, inclJpsiRAAError);
  TGraphErrors *gPromptJpsiRAA = new TGraphErrors(7, promptNpart, promptJpsiRAA, promptNpartError, promptJpsiRAAError);
  ...
  TGraphErrors *gInclJpsiRAASyst = new TGraphErrors(7, promptNpart, inclJpsiRAA, promptSystX, inclJpsiRAASyst);
  TGraphErrors *gPromptJpsiRAASyst = new TGraphErrors(7, promptNpart, promptJpsiRAA, promptSystX, promptJpsiRAASyst);
  ...
  TGraphErrors *gInclJpsiRAAP = new TGraphErrors(7, promptNpart, inclJpsiRAA, promptNpartError, promptNpartError);
  TGraphErrors *gPromptJpsiRAAP = new TGraphErrors(7, promptNpart, promptJpsiRAA, promptNpartError, promptNpartError);
  ...
  if (plotStar) {
    lumiStar = new TBox(380.0,0.9,399.0,1.10);
    lumiStar->SetFillStyle(0);
    lumiStar->SetLineWidth(2);
    lumiStar->SetLineColor(kBlack);

    lumiStarUps = new TBox(380.0,0.67,399.0,1.33);
    lumiStarUps->SetFillStyle(0);
    lumiStarUps->SetLineWidth(2);
    lumiStarUps->SetLineColor(kBlack);

    lumiStarUps2 = new TBox(380.0,0.886,399.0,1.114);
  ...
  load_STAR();
  pgStar->SetMarkerStyle(30);
  pgStar->SetMarkerColor(kGreen+2);
  pgStar->SetMarkerSize(2);
  ...
  if (plotPhenix) {
    ...
    //    leg1a = new TLegend(0.44,0.45,0.77,0.65);
    leg1a = new TLegend(0.49,0.50,0.82,0.65);
    leg1a->SetFillStyle(0);
    leg1a->SetFillColor(0);
    leg1a->SetBorderSize(0);
    leg1a->SetMargin(0.2);
  ...
  if (plotPhenix) {
    ...
    leg1a->SetHeader("AuAu  #sqrt{s_{NN}} = 200 GeV");
    leg1a->AddEntry(pgStar,"STAR preliminary","P");
    leg1a->AddEntry(pgStar,"p_{T} > 5 GeV/c, |y|<1.0","");
    leg1a->Draw();
  }
  leg1->Draw();
  ...
  if (plotPhenix) {
    ...
    c1->SaveAs("PromptJpsi_RAA_STAR.pdf");
    c1->SaveAs("PromptJpsi_RAA_STAR.png");
  }
  else {
  ...
  TLegend *leg2a;
  if (plotStar) {
    //    leg2a = new TLegend(0.44,0.45,0.77,0.65);
    leg2a = new TLegend(0.49,0.50,0.82,0.65);
    ...
    leg2a->SetHeader("AuAu  #sqrt{s_{NN}} = 200 GeV");
    leg2a->AddEntry(pgStarUps,"STAR preliminary","P");
    leg2a->AddEntry(pgStar,"|y|<0.5","");
    leg2a->Draw();
  }
  ...
  if (plotStar) {
    c2->SaveAs("Ups_RAA_STAR.pdf");
    c2->SaveAs("Ups_RAA_STAR.png");
  }
  else {
    c2->SaveAs("PromptNonPromptJpsi_Ups_RAA.pdf");
    c2->SaveAs("PromptNonPromptJpsi_Ups_RAA.png");
  }
